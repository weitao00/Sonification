...
import os
import random
from data import generate_wine_data
...
def main():
    ...
    max_per_beat = 8  # Increased maximum number of wines per beat
    tempo = 180  # Tempo in BPM
    duration = 0.4  # Note duration in beats

    # Define directories
    midi_dir = os.path.join('midi', 'wine')
    plots_dir = os.path.join('plots', 'wine')
    ...
    # Data Generation
    # ============================
    df_wine = generate_wine_data(num_wines=num_wines, seed=seed)
    ...
    # Assign a random pitch from the cluster's scale to each wine
    df_wine['pitch'] = df_wine['cluster'].apply(lambda x: random.choice(scales_midi[x]))

    # ============================
    # Normalizing PCA Components
    # ============================
    scaler_pca = MinMaxScaler()
    pca_scaled = scaler_pca.fit_transform(df_wine[['PCA1', 'PCA2']])
    df_wine['PCA1_scaled'] = pca_scaled[:, 0]
    df_wine['PCA2_scaled'] = pca_scaled[:, 1]

    # ============================
    # Mapping PCA Components to Musical Parameters
    # ============================
    df_wine['velocity'] = (110 + df_wine['PCA1_scaled'] * (127 - 110)).astype(int)  # Velocity range: 110-127

    # ============================
    # Controlled Timeframe Assignment
    # ============================
    df_realistic = assign_time_beats(df_wine, desired_duration_beats, max_per_beat=max_per_beat)
    ...
    midi_filename = os.path.join(midi_dir, 'wine_sonification_tenser.mid')
    ...
    fig = px.scatter(
        ...
        title='Wine Data Sonification Visualization',
    ...
    plot_html = os.path.join(plots_dir, 'wine_sonification_visualization.html')
    plot_png = os.path.join(plots_dir, 'wine_sonification_visualization.png')
